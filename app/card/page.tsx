...
function CardLayout({
  ...
  return (
    ...
    >
      <motion.div
        className={`absolute ${
          isMobile ? "top-[9%] left-4 right-9 text-center" : "top-[30%] left-[6rem] text-left"
        } text-white`}
      ...
      >
        ...
        <div className={`${isMobile ? "max-w-md mx-auto" : "max-w-3xl"}`}>
          ...
          >
            <Disc3
              size={isMobile ? 20 : 26}
              className={`shrink-0 ${isPlaying ? "animate-spin" : ""}`}
            />
      ...
      <GiggleImage
        ...
        customDelay={1}
        className={`absolute bottom-0 ${isMobile ? "left-32" : "right-[50rem]"}`}
      />
...
// Main card page component (wrapped in Suspense)
  ...
  if (!encoded) {
    return (
      <p className="text-center mt-10 text-red-600">
        Invalid or missing greeting details.
      </p>
    );
  }
